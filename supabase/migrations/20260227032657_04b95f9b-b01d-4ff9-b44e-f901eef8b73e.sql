...
BEGIN
  ...
  FROM licitacoes l
  LEFT JOIN licitacao_itens li2 ON li2.licitacao_id = l.id
  ...
  -- Re-sort by valor_estimado desc
  -- Note: DISTINCT ON requires ORDER BY on the same column first,
  -- so we wrap it
  RETURN;
END;
$$;

-- Now create the proper version that sorts by value
DROP FUNCTION IF EXISTS public.search_licitacoes(text,text,text,text,text,text,text,integer,integer,boolean,text);
...
BEGIN
  ...
  FROM (
    ...
    WHERE
      ...
      AND (p_date_from IS NULL OR l2.data_publicacao >= p_date_from)
      AND (p_date_to IS NULL OR l2.data_publicacao <= p_date_to)
      AND (NOT p_com_vencedor OR lv2.id IS NOT NULL)
      AND (p_vencedor IS NULL OR lv2.razao_social ILIKE '%' || p_vencedor || '%')
  ) sub;
