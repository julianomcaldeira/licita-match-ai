...
BEGIN
  ...
  IF cardinality(COALESCE(p_vencedores, '{}')) > 0 THEN
    v_sql := v_sql || ' AND EXISTS (
        ...
        JOIN licitacao_vencedores lv ON lv.item_id = li2.id
        WHERE li2.licitacao_id = l.id
          AND EXISTS (SELECT 1 FROM unnest($11) v WHERE lv.razao_social ILIKE ''%'' || trim(v) || ''%''))';
  END IF;

  v_sql := v_sql || ' ORDER BY ' || v_order || ' LIMIT $12 OFFSET $13';

  -- envelopa para resolver o vencedor principal apenas das linhas da página
  v_sql := '
  ...
  RETURN QUERY EXECUTE v_sql USING
    ...
    COALESCE(p_ufs, '{}'),
    COALESCE(p_situacoes, '{}'),
    COALESCE(p_modalidades, '{}'),
    COALESCE(p_orgaos, '{}'),
    COALESCE(p_com_vencedor, false),
    COALESCE(p_sem_resultado, false),
    v_terms,
    v_itens,
    COALESCE(p_vencedores, '{}'),
