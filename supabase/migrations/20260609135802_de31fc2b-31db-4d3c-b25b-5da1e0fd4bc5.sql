...
-- Novos campos de destaques do mês
ALTER TABLE public.indice_startgi_mensal
  ADD COLUMN IF NOT EXISTS top_orgao_nome text,
  ADD COLUMN IF NOT EXISTS top_orgao_valor numeric(18,2),
  ADD COLUMN IF NOT EXISTS top_fornecedor_nome text,
  ADD COLUMN IF NOT EXISTS top_fornecedor_valor numeric(18,2),
  ADD COLUMN IF NOT EXISTS top_modalidade text,
  ADD COLUMN IF NOT EXISTS top_modalidade_share numeric(5,1),
  ADD COLUMN IF NOT EXISTS maior_contrato_valor numeric(18,2),
  ADD COLUMN IF NOT EXISTS maior_contrato_objeto text,
  ADD COLUMN IF NOT EXISTS ticket_medio numeric(18,2),
  ADD COLUMN IF NOT EXISTS orgaos_unicos integer,
  ADD COLUMN IF NOT EXISTS fornecedores_unicos integer;
...
-- Atualiza trigger para copiar os novos campos no histórico
...
-- Função de cálculo enriquecida com destaques do mês
...
BEGIN
  ...
  IF v_valor > 0 THEN
    ...
    -- Destaque segmento (maior variação MoM)
    WITH cur AS (
      SELECT
        CASE
          WHEN objeto ILIKE '%saúde%' OR objeto ILIKE '%saude%' OR objeto ILIKE '%hospital%' OR objeto ILIKE '%medicament%' OR objeto ILIKE '%clínic%' OR objeto ILIKE '%clinic%' THEN 'Saúde'
    ...
    SELECT c.bucket,
           CASE WHEN COALESCE(p.v,0) > 0 THEN ROUND(((c.v - p.v) / p.v) * 100, 2) END
      INTO v_destaque, v_destaque_var
    FROM cur c LEFT JOIN prev p USING (bucket)
    WHERE c.bucket <> 'Outros'
    ORDER BY (CASE WHEN COALESCE(p.v,0) > 0 THEN ((c.v - p.v) / p.v) END) DESC NULLS LAST
    LIMIT 1;
    ...
    -- Ticket médio + nº órgãos + nº fornecedores
