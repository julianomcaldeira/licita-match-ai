...
DROP FUNCTION IF EXISTS public.search_licitacoes(text,text,text,text,text,text,text,text,boolean,integer,integer,boolean);
  ...
  IF p_vencedor IS NOT NULL AND trim(p_vencedor) <> '' THEN
    ...
    filtered AS (
      ...
      WHERE (p_date_from IS NULL OR l.data_publicacao >= p_date_from::date)
        ...
        AND (NOT COALESCE(p_sem_resultado, false) OR l.valor_homologado IS NULL OR l.valor_homologado = 0)
