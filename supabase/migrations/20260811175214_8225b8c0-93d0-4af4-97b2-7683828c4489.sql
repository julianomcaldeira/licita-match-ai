...
AS $$
  WITH base AS (
    SELECT l.fonte,
           ...
           max(l.created_at) FILTER (WHERE l.status IN ('sucesso','parcial')) AS ultimo_sucesso,
           count(*) FILTER (WHERE l.created_at > now() - interval '24 hours') AS execucoes_24h,
           count(*) FILTER (WHERE l.created_at > now() - interval '24 hours' AND l.status NOT IN ('sucesso','parcial')) AS erros_24h,
