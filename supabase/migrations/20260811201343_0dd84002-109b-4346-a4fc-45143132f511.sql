ALTER TABLE public.indice_startgi_mensal
  ADD COLUMN IF NOT EXISTS outliers_excluidos integer NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS valor_outliers numeric NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS teto_outlier numeric;
...
BEGIN
  ...
  -- Corte de outliers: erros de digitação na fonte oficial distorcem o índice.
  SELECT percentile_disc(0.999) WITHIN GROUP (ORDER BY v) INTO v_p999 FROM _ctr WHERE v > 0;
  v_teto := GREATEST(COALESCE(v_p999, 0) * 20, 1000000000);

  SELECT COUNT(*), COALESCE(SUM(v), 0) INTO v_out_qtd, v_out_valor
  FROM _ctr WHERE is_cur AND v > v_teto;

  DELETE FROM _ctr WHERE v > v_teto;
  ANALYZE _ctr;
  ...
  IF v_base IS NULL THEN
    ...
    ELSE
      ...
      WHERE COALESCE(data_assinatura, data_publicacao) >= date '2024-01-01'
        ...
        AND COALESCE(valor_inicial, 0) <= v_teto;
    END IF;
  END IF;
  ...
  INSERT INTO public.indice_startgi_mensal AS t (
    ...
    outliers_excluidos, valor_outliers, teto_outlier
  ) VALUES (
  ...
  ON CONFLICT (mes_referencia) DO UPDATE SET
    ...
    outliers_excluidos = EXCLUDED.outliers_excluidos,
    valor_outliers = EXCLUDED.valor_outliers,
    teto_outlier = EXCLUDED.teto_outlier
...
SELECT public.compute_indice_startgi('2026-06', true);
SELECT public.compute_indice_startgi('2026-07', true);