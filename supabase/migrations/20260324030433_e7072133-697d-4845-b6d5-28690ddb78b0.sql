...
BEGIN
  ...
  WHERE
    (search_words IS NULL OR (
      NOT EXISTS (
        SELECT 1 FROM unnest(search_words) w WHERE lower(l.objeto) NOT LIKE '%' || w || '%'
      )
      OR EXISTS (
        SELECT 1 FROM licitacao_itens li_s
        WHERE li_s.licitacao_id = l.id
        AND NOT EXISTS (
          SELECT 1 FROM unnest(search_words) w WHERE lower(li_s.descricao) NOT LIKE '%' || w || '%'
        )
      )
    ))
    ...
    AND (
      NOT p_com_vencedor
      OR l.valor_homologado IS NOT NULL AND l.valor_homologado > 0
    )
    AND (
      p_vencedor IS NULL
      OR EXISTS (
