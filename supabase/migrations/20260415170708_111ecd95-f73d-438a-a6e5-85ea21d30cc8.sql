...
-- Fix ingestao_logs: restrict read to admin_central only
DROP POLICY IF EXISTS "Authenticated users can read ingestao_logs" ON public.ingestao_logs;

CREATE POLICY "Admin central can read ingestao_logs"
  ON public.ingestao_logs
  FOR SELECT
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin_central'::app_role));

-- Fix search_licitacoes: set search_path
DO $$ BEGIN
  DROP FUNCTION IF EXISTS public.search_licitacoes(text, text, text, text, text, text, text, boolean, boolean, integer, integer, text);
