...
import {
  Search, Zap, Building2, Brain, BarChart3, ArrowRight, TrendingUp, Activity,
...
export default function DashboardOverview() {
  ...
  const { data: topOps } = useQuery({
    ...
    queryFn: async () => {
      let q = supabase.from("oportunidades")
        ...
        .order("score_aderencia", { ascending: false }).limit(5);
      if (!isAdmin && empresaId) q = q.eq("empresa_id", empresaId);
  ...
  const { data: ufStats } = useQuery({
    ...
    queryFn: async () => {
      let q = supabase.from("oportunidades").select("licitacoes(uf)").gte("score_aderencia", 50);
      if (!isAdmin && empresaId) q = q.eq("empresa_id", empresaId);
      const { data } = await q;
      ...
      return Object.entries(ufMap).sort((a, b) => b[1] - a[1]).slice(0, 8);
    },
  });
      ...
      <div className="relative overflow-hidden rounded-2xl p-6 sm:p-8" style={{ background: "var(--gradient-hero)" }}>
        ...
        <div className="relative z-10">
          ...
          <p className="mt-1 text-sm sm:text-base text-primary-foreground/80">
            {isAdmin ? "Visão geral de todas as empresas e oportunidades" : "Suas oportunidades e análises de licitações"}
          </p>
        </div>
      </div>
          ...
          <div className="mt-4 flex items-center justify-center gap-6">
            ...
            <div className="text-center">
              ...
              <p className="mt-2 font-display text-xl font-bold text-foreground">
                {(stats?.totalOps ?? 0) - (stats?.riskLow ?? 0)}
              </p>
          ...
          <div className="space-y-2">
            {topOps?.length ? topOps.map((op: any) => (
              ...
              <div className="flex flex-col items-center py-8">
                ...
                <Link to="/oportunidades" className="mt-2 text-xs text-primary hover:underline">Analisar agora →</Link>
              </div>
            )}
          </div>
        </motion.div>
